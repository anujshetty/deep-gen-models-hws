...
class SSVAE(nn.Module):
    ...
    def negative_elbo_bound(self, x):
        ...
        log_uniform_prob = torch.log(torch.ones_like(y_prob) / self.y_dim)

        # Duplicate y based on x's batch size. Then duplicate x
        # This enumerates all possible combination of x with labels (0, 1, ..., 9)
        y = np.repeat(np.arange(self.y_dim), batch_size)
        y = x.new(np.eye(self.y_dim)[y])
        x = ut.duplicate(x, self.y_dim)

        # Compute negative Evidence Lower Bound and its KL_Z, KL_Y and Rec decomposition
        z_mu, z_logvar = self.enc(x, y)
        z = ut.sample_gaussian(z_mu, z_logvar)
        kl_z = ut.kl_normal(z_mu, z_logvar, self.z_prior[0], self.z_prior[1]).reshape((batch_size, self.y_dim))
        e_kl_z = (y_prob*kl_z).sum(dim=1).mean()

        logits = self.dec(z, y)
        rec =  -ut.log_bernoulli_with_logits(x, logits).reshape((batch_size, self.y_dim))
        e_rec = (y_prob*rec).sum(dim=1).mean()

        kl_y = ut.kl_cat(y_prob, y_logprob, log_uniform_prob).mean()
        nelbo = kl_y + (y_prob*(kl_z+rec)).sum(dim=1).mean()        

        ################################################################################
        # End of code modification
        ################################################################################
        return nelbo, e_kl_z, kl_y, e_rec
