...
def sample_gaussian(m, v):
    ...
    epsilon = torch.randn_like(m)  # samples from a standard normal distribution
    z = m + epsilon * torch.sqrt(v)  # element-wise operations
...
def log_normal(x, m, v):
    ...
    # log_normal = -0.5 * (torch.log(2 * torch.tensor(np.pi)) + torch.log(v) + ((x - m) ** 2) / v)
    # log_prob = torch.sum(log_normal, dim=-1)
    log_probs = torch.distributions.Normal(m, torch.sqrt(v)).log_prob(x.unsqueeze(0))
...
def log_normal_mixture(z, m, v):
    ...
    log_probs = torch.distributions.Normal(m, torch.sqrt(v)).log_prob(z.unsqueeze(-2))
    log_prob = log_mean_exp(log_probs, dim=-2)
