...
class MADE(nn.Module):
    ...
    def forward(self, z):
        ...
        # Pass input z through the network
        log_det = torch.zeros(z.size(0))
        for i in range(self.input_size):
            network_output = self.net(x)
            mu, alpha = network_output.chunk(2, dim=1)
    ...
    def inverse(self, x):
        ...
        # Split the network output into mu and alpha
        mu, alpha = network_output.chunk(2, dim=1)
        
        # Calculate z using the inverse transformation
        z = (x - mu) * torch.exp(-alpha)
        
        # Calculate log_det using the sum of alphas, negated
...
class MAF(nn.Module):
    ...
    def log_probs(self, x):
        ...
        z = x
        for flow in self.nf:
            # Each flow is a MADE followed by a PermuteLayer
            if isinstance(flow, MADE):
                # Transform z and update log_prob with log determinant
                z, log_det_jacobian = flow.inverse(z)
                log_prob += log_det_jacobian
            else:  # PermuteLayer
                z, _ = flow.inverse(z)

        # Now z is in the base distribution, compute log_prob under base_dist
